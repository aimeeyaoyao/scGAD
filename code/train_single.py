...
def dataset_spliting(X, cellname, class_set, labeled_number, labeled_ratio=0.5, random_seed=8888):
    shared_class_set = class_set[:labeled_number]
    source_index = []
    target_index = []
    np.random.seed(random_seed)

    for i in range(X.shape[0]):
        if cellname[i] in shared_class_set and np.random.rand() < labeled_ratio:
            source_index.append(i)
        elif cellname[i] in class_set:
    ...
    for j in range(len(shared_class_set), len(class_set)):
        target_Y[target_cellname == class_set[j]] = j
...
if __name__ == "__main__":
    ...
    filename_set = ["Cao", "Hochane", "Park", "Quake_10x", "Quake_Smart-seq2", "Wagner", "Zeisel_2018", "Zheng", "Chen", "Guo"]
    ...
    for i in range(args.num, args.num + 1):
        ...
        shared_classes = int(len(class_set) / 2)
        ...
        source_X, source_cellname, source_Y, source_batch, target_X, target_cellname, target_Y, target_batch \
            = dataset_spliting(X, cell_name, class_set, shared_classes, labeled_ratio=labeled_ratio, random_seed=args.random_seed)
        print("The shape of source data is {}, and the shape of target data is {}".format(source_X.shape, target_X.shape))

        seen_classes_set = class_set[:shared_classes]
        novel_classes_set = class_set[shared_classes:]
        ...
        for epoch in range(args.epochs + 1):
            if epoch % args.interval == 0:
                ...
                print(
                    "Currently, we have the best overall acc is {:.4f}, best seen acc is {:.4f}, best unseen acc is {:.4f}"
                    ".".format(best_overall_acc, best_seen_acc, best_unseen_acc))
                print(
                    "Currently, we have the best overall acc2 is {:.4f}, best seen acc2 is {:.4f}, best unseen acc2 is {:.4f}"
                    ".".format(best_overall_acc2, best_seen_acc2, best_unseen_acc2))
                ...
                if epoch == args.epochs:
                    ...
                    print(
                        "After training, we have the final overall acc is {:.4f}, final seen acc is {:.4f}, final unseen acc is {:.4f}, "
                        ".".format(final_overall_acc, final_seen_acc,
                            final_unseen_acc))
                    print(
                        "After training, we have the final overall acc2 is {:.4f}, final seen acc2 is {:.4f}, final unseen acc2 is {:.4f}, "
                        ".".format(final_overall_acc2, final_seen_acc2,
                            final_unseen_acc2))
            ...
            for batch_idx, (x_t, raw_x_t, sf_t, y_t, index_t, batch_t) in enumerate(target_dataloader):
                ...
                recon_loss = recon_loss - torch.mean(mask_all[:labeled_len] * torch.log(mask_s2) + (1. - mask_all[:labeled_len]) * torch.log(1. - mask_s2)) \
                             - torch.mean(mask_all[labeled_len:] * torch.log(mask_t2) + (1. - mask_all[labeled_len:]) * torch.log(1. - mask_t2))
                ...
                labeled_onehot = F.one_hot(train_targets, num_classes = shared_classes)[index_s].float()
                unlabeled_onehot = F.one_hot(test_label, num_classes = total_classes)[index_t].float()
                ...
                if epoch < args.pretrain:
                    ...
                    loss = recon_loss + ce_loss + align_loss + pse_loss
                else:
                    loss = recon_loss + ce_loss + align_loss + pse_loss
            ...
            print("Training {}/{}, zinb loss: {:.4f}, ce loss: {:.4f}, pse loss: {:.4f}, "
                  "align loss: {:.4f}, clu loss: {:.4f}".format(epoch, args.epochs, recon_losses.avg,
                                              ce_losses.avg, pse_losses.avg, align_losses.avg, clu_losses.avg))
        result_list.append([filename, shared_classes, total_classes, labeled_ratio,
